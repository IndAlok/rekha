...
Desk is a Next app in `apps/web`. Point `API_UPSTREAM` at the API. API is the Docker image in `infra/Dockerfile.api`. Set `REKHA_ENV`, `OPS_TOKEN`, `RAZORPAY_WEBHOOK_SECRET`, and `CORS_ORIGINS` before you expose it.
